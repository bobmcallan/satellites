...
System-scope lifecycle contracts (preplan, plan, story_close) are review-only. They must not embed user/project operations like git, merge, push, commit, CI watch, or deploy. Their skills are also scope=system; both are managed through seed files, not MCP mutations.
