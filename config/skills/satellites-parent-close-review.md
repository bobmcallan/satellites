...
enacts your verdict. You write NOTHING — no ledger rows, no file, no document body.
...
the anchor's `## Workflow` and JUDGE only — you write nothing (no ledger rows, no
file or document mutation); the client enacts your verdict. This reviewer is
PRODUCT machinery (system scope) — it reads cleanly in any repo and leans on no
repo-dev specifics.
...
guardrails:
  always:
    ...
    - Verify the anchor's ## Workflow declares a transition whose from == story_status AND reviewer_skill == satellites-parent-close-review — if none exists, reject; the client enacts that edge (→ done) on your accept.
  ask_first: []
  never:
    - Accept (close the anchor) with a non-terminal child, an unreadable check, or no children.
    - Write anything at all — no ledger_append, no document_upsert, no other mutating exec; emit only the final {decision, notes} JSON as output.
```
