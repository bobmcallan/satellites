...
- **System seed registry**: agent-facing prose that ships with the binary (install schema, MCP load-context, system-variables taxonomy, any future reviewer/summary skeleton) lives under `config/seed/system/` as markdown and is embedded into `satellites-server` via `go:embed`. On every boot, the embedded bytes are reconciled against the `system_seeds` table (`name`, `body`, `embedded_hash`, `applied_at`) — missing rows are inserted, drifted rows are replaced and `applied_at` is advanced, matching rows are a zero-write no-op. Reconciliation also mirrors the body into the `documents` table at scope=`system` so `document_get` remains the read API. No verb writes to `system_seeds` and the documents store rejects scope=`system` writes from non-seed callers (`ErrScopeReadonly`); the only path that mutates either is the boot reconciler running against immutable embed.FS bytes. Changing a system seed therefore requires editing the markdown in `config/seed/system/` and shipping a binary release — the rev couples the prose to the version of the server, making it structurally impossible for "which prompt is running where" to drift across environments. Operators audit the registry via the read-only `system_seeds_list` verb.
