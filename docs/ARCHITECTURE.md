...
- **Thin MCP + CLI gateway.** Single execution path (CLI verbs) for all primaries. MCP exposes exactly one tool — `document_get` — which is enough for an agent to fetch the install schema and bootstrap a local CLI; everything else is dispatched through that CLI. Gemini/Codex/Warp call the same verbs via Bash → CLI. No second-class primaries.
