...
- **CLI distribution**: `.version` (at repo root) is the canonical version source — `version: X.Y.Z` plus `build: <UTC>` timestamp, auto-bumped on every commit by the `/commit-push` skill. The release workflow triggers on each push to `main`, reads `.version`, and if no matching `v<version>` tag exists it builds the platform matrix ((linux, darwin) × (amd64, arm64) for `satellites`; linux/amd64 for `satellites-server`), creates the tag, and publishes the GitHub release with `.sha256` checksums alongside each binary. The MCP `satellites_init` verb returns an install payload pointing at the GitHub "latest release" alias; agents install the binary into `.satellites/` colocated under the consumer project root.
