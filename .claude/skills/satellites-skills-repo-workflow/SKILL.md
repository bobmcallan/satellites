...
5. When the artifact is authored, reviewed, and published, request the
   checkpoint: `satellites story status_transition <story-id> --skill
   satellites-story-done-review`. The client enacts the ungated checkpoint edge
   `in_progress → done-review`, then the done-review gate JUDGES the published
   artifact against the story and the client enacts pass → `done` / fail →
   `in_progress` (×3, then `blocked`). A skills repo has no code build/test
   traverse — the publish content gate (run by `skill publish` / `principle
   upload`) is the fail-closed check, and done-review is the verdict.
...
- `in_progress` (executor) — author the skill/principle, review it, publish it; every fail edge lands back here. The checkpoint leaves here for `done-review` once the artifact is published.
...
A skills repo has no Go build/unit/integration traverse, so this workflow names
no code-traverse state. The fail-closed check is the **publish content gate**
run by `satellites skill publish` / `satellites principle upload` (strict
drift/reference review before an artifact reaches the library); `done-review` is
the verdict on the story. When the change touches process configuration (this
workflow, a gate or capability skill, a principle), run
[[satellites-workflow-drift-review]] before pushing.
...
guardrails:
  always:
    ...
    - Publish through `skill publish` / `principle upload` so the content gate runs before the artifact reaches the library.
  ask_first:
