...
| `satellites init` | Make a repo ready for satellites, idempotently — ensure `.satellites/`, a `satellites.toml`, the PreToolUse START-door + advisory hooks in `.claude/settings.json`, a `.gitignore` managed block for `.satellites/` local state, and the gateless baseline workflow in `.satellites/workflows/` (create-if-absent). After scaffolding it runs the fast (static) `validate` pass (same code path as `satellites validate`) and prints any STALE/UNRESOLVABLE governance with a remedy — so drift an out-of-session `satellites update` introduced surfaces here instead of at the next gate; non-fatal, `--skip-validate` to opt out. Re-run when hooks change; existing files/settings are preserved. | DONE. |
...
| `satellites clear --kind <principle\|skill\|workflow\|all>` | Archive (soft-tombstone) the caller's PROJECT-scoped governance rows — principles, skills, the `kind:workflow/gate/reviewer` skills — so a project can be rebuilt on an updated satellites. A pure client-side composition of `document_list` + `document_delete` ([[no-new-mcp-verbs]]); the first-class replacement for scripting raw `exec document_delete`. Rails: project scope only (never system/shared rows like `agent-goals`), stories are never touched (they hard-delete), `--dryrun` enumerates without writing. Confirmation is TTY-aware: at an interactive terminal a real run prompts `[y/N]`; when stdin is NOT a terminal (an agent via its harness, CI, a pipe) the CLI cannot prompt so clear REFUSES — re-run with `--force` (canonical; `--yes` is a retained alias). For an agent the operator's real confirmation gate is the harness permission prompt on the command, not a CLI prompt. Tombstones are recoverable and local `.satellites/` sources are left on disk. | DONE. |
| `satellites validate` (alias `doctor`) | Read-only diagnosis of the caller's PROJECT governance (principles/skills/workflows) against the CURRENT binary — a per-artifact verdict OK / STALE / UNRESOLVABLE. Composes the `workflow check` resolver (every workflow's reviewer/gate reference must resolve embed→local→server) as the fast default static pass; `--deep` adds the `claude -p` content review over skills + principles (report-only). Exits non-zero on any UNRESOLVABLE so it gates CI and `init`. Surfaces the drift an out-of-session `satellites update` introduces (a workflow naming a reviewer the new binary can't resolve) before a gate fails ([[no-new-mcp-verbs]], [[process-as-configuration]]). | DONE. |
...
| `satellites context validate [--semantic] [--strict] [--json]` | Validate the WHOLE substrate CORPUS-WIDE in one consolidated report (no story-id) — runs the structural context-review checks (`reviewContextConflicts`) over every non-terminal story's `## Workflow`, sizes the always-on surface against its baseline (`context budget` machinery), and with `--semantic` runs the `satellites-context-conflict-review` reviewer once over the corpus bundle. Each finding carries a descriptive fix action for the repair loop. Operator/CI-facing, out of band, writes no ledger rows; `--strict` exits non-zero on any error-severity finding (epic:context-validation). | DONE. |
| `satellites context repair [--apply] [--json]` | The report→repair loop over the corpus validation findings (epic:context-validation). Reuses the SAME findings `context validate` produces and classifies each into `auto` (a started story's missing/broken `## Workflow` → stamp via `workflow embed`), `expected` (a pre-start story has no embedded workflow yet — not a defect), or `manual` (names the system skill/command: `skill sync`, `corpus_review`, `context curate`). Default is a DRY-RUN plan; `--apply` executes only the auto actions (reusing `runWorkflowEmbed`, no re-implementation) then re-validates and reports the before→after counts. Operator-facing, out of band, no ledger rows; `--apply` mutates the corpus and is opt-in. | DONE. |
...
| `satellites story get <id> [--body]` | Server-side story read — status, state actor (derived from the story's embedded `## Workflow`), category, priority, tags, parent, headline. `--body` also prints the full story body (plan + `## Workflow`) the command already fetches, so reading a story's plan is one command. The server complement of `satellites work status` (the local live-engagement view). Read-only. **A story body is read BY ID** — `satellites story get <id> --body`, or the `document_get id=<sty_id>` verb — never by `(scope, name)`, which returns a scope-coherence violation (a story is not a key-addressed document). | DONE. |
